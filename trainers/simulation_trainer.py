...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for step, data in enumerate(bar, 1):
            ...
            data_ready = self._data_ready(data)
    ...
    def _validation_epoch(self, epoch):
        ...
        self.validation_dataset.set_epoch(epoch)
        for step, data in enumerate(bar, 1):
            # data = (target_reverb, dis_info)
            data_ready = self._data_ready(data)
            input_audio = data_ready['input'] # [b, wav length]
            tgt_dis = data_ready['target_distance'] # [b, 1]
            
            # model
            output = self.model(input_audio, 0)  
            
            # loss
            loss = self.loss_func(output, tgt_dis)
            loss = loss.mean()
            ...
            if step <= 3:
                ...
                info_used.append({'target dis': tgt_dis.cpu().numpy().tolist()})
    ...
    def _data_ready(self, data):
        # data = ( target_reverb, dis_info)
        # input: target_reverb
        input_audio = data[0].to(self.device).squeeze(1) # [b, wav length]  # 1
        if self.resample:
